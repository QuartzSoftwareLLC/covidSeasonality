#' Plotter
#' Plots covid seasonality by locales
#' @param locales the country of interest
#' @param decomposition.method the decomposition method for anomalize
#' @param anomalize.method the anomalize method for anomalize
#' @import janitor wppExplorer cowplot anomalize prophet ggplot2 tidyr dplyr
plotter <- function(locales = NULL, decomposition.method = "twitter", anomalize.method = "gesd") {
    shiny.covidSeasonality::df %>%
        subset(
            location %in% locales
        ) %>%
        dplyr::select(
    ...
    pops <- lapply(locales, function(x) wpp.by.country(wpp.indicator("tpop"), shiny.covidSeasonality::iso_codes$charcode[shiny.covidSeasonality::iso_codes$name %in% x]))
    ...
    df.prophet <- tibble::column_to_rownames(df.prophet, var = "ds")
    ...
    ggplot(data = yo) +
        ...
        geom_rect(
            data = label_range, fill = "#A66923", color = "#f2f2f2", alpha = 0.5,
            aes(
                xmin = xmin, xmax = xmax,
        ...
        geom_text(
            data = label_range,
            aes(
                x = xmin + 25, y = ymin + 500,
                group = year, label = year
            )
        ) +
        geom_ribbon(
            aes(x = ds, ymin = recomposed_l1, ymax = recomposed_l2),
            color = "#272936", alpha = 0.2
        ) +
        scale_x_date(
            date_breaks = "1 month",
            date_labels = "%b",
